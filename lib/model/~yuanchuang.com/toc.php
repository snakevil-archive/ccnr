<?php
/**
 * Represents as a novel TOC page in `yuanchuang.com'.
 *
...
class TOC extends Model\TOC
{
     ...
     * @var string
     */
    const PATTERN = '~^http://www\.yuanchuang\.com/bookcatalog/\d+\.html$~';
    ...
    protected function parse($content)
    {
        ...
        $this->title = $s_ret;
        $s_ret = $this->crop('@<span><strong>作者：</strong><a class="coffee" target="_blank" href="http://space.yuanchuang.com/Home/UHome/\d+">@', '@</a>@', $content);
        if (false === $s_ret)
            throw new Model\AuthorNotFoundException;
        $this->author = $s_ret;
        $s_ret = $this->crop('@<div class="title3">@', '@你可能喜欢@', $content);
        if (false === $s_ret)
            throw new Model\ChaptersListingNotFoundException;
        $content = '<div class="title3">' . $s_ret;
        $this->chapters = array();
        $s_prefix = '..';
        do
        {
            $s_vol = $this->crop('@<div class="title3">@', '@<a class="red" target="_blank" href="@', $content);
            if (false === $s_vol)
                break;
            $s_ret = $this->crop('@<ul>@', '@</ul>@', $content);
            if (false === $s_ret ||
                false === preg_match_all('@<li><a href="http://www.yuanchuang.com(/bookreader/\d+/\d+\.html)".*>(.*)</a></li>@U', $s_ret, $a_tmp)
            )
